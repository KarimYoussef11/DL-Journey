...
def count_tensor_elements(x: Tensor) -> int:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    l = torch.Tensor([i for i in x.shape])
    num_elements = int(torch.prod(l))
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return num_elements
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    # Replace "pass" statement with your code
    x = torch.tensor(x)
    y = torch.nn.functional.one_hot(x, num_classes=len(x)+1)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mean = torch.sum(x, 0) / x.shape[0]
    std = torch.sqrt(torch.sum((x - mean) ** 2, 0) / (x.shape[0] - 1)) # minus 1 for the unbiased version
    y = (x - mean) / std
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def mm_on_gpu(x: Tensor, w: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    cuda_device_gpu = torch.device("cuda:0")
    device_cpu = torch.device("cpu")
    x.to(cuda_device_gpu)
    w.to(cuda_device_gpu)
    y = torch.matmul(x, w)
    y.to(device_cpu)
    x.to(device_cpu)
    w.to(device_cpu)
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = torch.tensor(list(map(sum, xs))) / ls
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
