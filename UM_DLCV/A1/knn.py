...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for j in range(num_test):
        dists[:, j] = torch.sum(((x_train - x_test[j]) ** 2), dim=1)
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    dists = torch.sum(x_train ** 2, dim=1).reshape(-1, 1) + torch.sum(x_test ** 2, dim=1).reshape(1, -1) - 2 * torch.matmul(x_train, x_test.T)
    # Note that the above implementation uses sqrt(a^2 + b^2 - 2* a* b)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    vals = torch.zeros(num_test, k).T
    inds = torch.zeros(num_test, k).T

    for i in range(num_test):
        vals[:, i], inds[:, i] = torch.topk(dists[:, i], k, largest=False)
    y_labels = y_train[inds.long()]
    for i in range(num_test):
        y_pred[i] = torch.mode(y_labels[:, i])[0]
...
def knn_cross_validate(
    ...
    for k_val in k_choices:
        for i in range(num_folds):
            xx = torch.cat([x_train_folds[j] for j in range(num_folds) if i != j])
            yy = torch.cat([y_train_folds[j] for j in range(num_folds) if i != j])
            xx_val = x_train_folds[i]
            yy_val = y_train_folds[i]
            KnnClass = KnnClassifier(xx, yy)
            acs.append(KnnClass.check_accuracy(xx_val, yy_val, k_val, quiet=True))
        k_to_accuracies.update({k_val: acs})
        acs = []
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies.keys():
        acc_new = torch.mean(torch.tensor(k_to_accuracies[k]))
        if acc_new > mean_acc:
            mean_acc = acc_new
            best_k = k
        elif acc_new == mean_acc:
            if k < best_k:
                best_k = k
